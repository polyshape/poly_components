...
function TokensTool() {
  ...
  const tokens = (globalTokens as any) || {};
  ...
  const ColorPick = ({ token, fallback }: { token: string; fallback: string }) => {
    const live = (tokens as any)[token] as string | undefined;
    const initial = /^#([0-9a-f]{3,8})$/i.test(live || '') ? (live as string) : fallback;
    const [val, setVal] = useState<string>(initial);
    // Keep picker in sync if globals change externally
    React.useEffect(() => {
      const next = /^#([0-9a-f]{3,8})$/i.test((tokens as any)[token] || '')
        ? ((tokens as any)[token] as string)
        : fallback;
      setVal(next);
