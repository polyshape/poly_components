...
import type { ModalProps } from "../src/modal/Modal.js";
...
function ModalRender(args: ModalProps) {
      ...
      <div style={{ padding: 16 }}>
        <div style={{ display: "grid", gap: 8, marginBottom: 12 }}>
          <label style={{ display: "grid", gap: 6 }}>
            <span>Modal content (HTML string):</span>
            <textarea
              value={content}
              wrap="off"
              spellCheck={false}
...
export const Basic: Story = {
  ...
  render: (args) => <ModalRender {...args} />,
};

function CustomCloseIconElementRender(args: ModalProps) {
  const [open, setOpen] = useState(false);
  return (
    <div style={{ padding: 16 }}>
      <Button appearance="primary" onClick={() => setOpen(true)}>Open</Button>
...
function LifecycleCallbacksRender(args: ModalProps) {
  const [open, setOpen] = useState(false);
  ...
  return (
    <div style={{ padding: 16, display: "grid", gap: 12 }}>
      ...
      <Modal {...args} isOpen={open} onClose={() => setOpen(false)} onAfterOpen={() => push("after-open")} onAfterClose={() => push("after-close")}>
        <p>Open/close the modal and watch events below.</p>
      </Modal>
...
function RequestCloseAndModelessRender(args: ModalProps) {
  const [open, setOpen] = useState(false);
  ...
  return (
    <div style={{ padding: 16, display: "grid", gap: 12 }}>
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
        <label><input type="checkbox" checked={blockBackdrop} onChange={(e) => setBlockBackdrop(e.target.checked)} /> Block backdrop</label>
        <label><input type="checkbox" checked={blockEscape} onChange={(e) => setBlockEscape(e.target.checked)} /> Block Esc</label>
        <label><input type="checkbox" checked={blockButton} onChange={(e) => setBlockButton(e.target.checked)} /> Block button</label>
      </div>
      <Modal
        {...args}
        isOpen={open}
        ...
        title="Request close"
      >
        <p>Try backdrop, Esc, and header close button.</p>
      </Modal>
      <Modal
        {...args}
        isOpen={open}
        onClose={() => setOpen(false)}
        title="Modeless example"
        modeless
        footer={<div>Background remains clickable.</div>}
      >
        <p>This modal does not dim or block the page behind. Enable draggable to move it.</p>
      </Modal>
    </div>
  );
}

export const RequestCloseAndModeless: Story = {
  args: { width: 560 },
  parameters: { controls: { exclude: ["trapFocus", "closeOnBackdropClick"] } },
  render: (args) => <RequestCloseAndModelessRender {...args} />,
};
...
function KeepMountedStateRender(args: ModalProps) {
  const [open, setOpen] = useState(false);
  const visible = typeof args.isOpen === "boolean" ? args.isOpen : open;
  return (
    <div style={{ padding: 16, display: "grid", gap: 12 }}>
...
function ModelessRender(args: ModalProps) {
  const [open, setOpen] = useState(false);
  ...
  return (
    <div style={{ padding: 16, display: "grid", gap: 12 }}>
      ...
      <Modal
        ...
        isOpen={visible}
        onClose={() => setOpen(false)}
        title={args.title ?? "Modeless Modal"}
        modeless
        footer={<div>Try clicking the background button while open.</div>}
      >
        <p>This modal does not dim or block the page.</p>
      </Modal>
    </div>
  );
}
...
function RequestCloseAndNestedRender(args: ModalProps) {
